...
**Демо:** [http://93.77.164.66/](http://93.77.164.66/) – живой деплой, там можно посмотреть реализацию в браузере.

> **Поиск жилья:** в каталоге (`db.json`) объекты есть только в двух городах – **Санкт-Петербург** и **Москва**. Чтобы увидеть карточки на странице результатов, введите одно из этих названий в поле города.
...
Живая версия проекта: **[http://93.77.164.66/](http://93.77.164.66/)**

Что можно проверить на демо:

- главная, поиск, карточки объектов, детальная страница;
- избранное, регистрация/логин, профиль, бронирование;
- переключение языка ru/en, мобильная вёрстка.

**Как найти объекты через поиск:** в базе только **Санкт-Петербург** и **Москва**. Другие города результатов не дадут – введите одно из этих названий (можно выбрать из автокомплита), укажите даты и нажмите «Искать».
...
- **Города в каталоге:** только **Санкт-Петербург** и **Москва** (`db.json`); для карточек на `/search` нужно указать одно из них
